/* ============================================================
   S=0 — The Sections of a Cone  ·  application logic
   ============================================================ */
(function () {
  'use strict';
  ...
  const THEME_META = {
    ...
    ellipse:      { glyph:'x²/a²', blurb:'The ellipse — the auxiliary circle and eccentric angle, tangents and normals, the director circle, the sum-of-focal-radii definition, and conjugate diameters.' },
    hyperbola:    { glyph:'x²−y²', blurb:'The hyperbola — asymptotes, the conjugate and rectangular hyperbola, tangents and normals, the difference-of-focal-radii definition, and the xy=c² parametrisation.' },
    tangents:     { glyph:'T=0', blurb:'The unified tangent–chord machinery across every conic: T=0 for the tangent and chord of contact, T=S₁ for the chord with a given midpoint, and SS₁=T² for the pair of tangents.' },
    polepolar:    { glyph:'T=S₁', blurb:'Pole and polar — the polar of a point, conjugate points and conjugate lines, the self-polar triangle, and the harmonic properties they encode.' },
    generalconic: { glyph:'B²−4AC', blurb:'The general second-degree equation Ax²+Bxy+Cy²+Dx+Ey+F=0 — classifying it by the discriminant, the condition for a pair of straight lines, the centre, and the axes.' },
  ...
  // ---- store ----
  function loadStore() {
    try { return JSON.parse(localStorage.getItem('conics_masterbook') || '{}'); }
    catch (e) { return {}; }
  }
  function saveStore() {
    try { localStorage.setItem('conics_masterbook', JSON.stringify(store)); } catch (e) {}
  }
  ...
  function renderHome(view){
    ...
    const fam=[['circle','The circle','x²+y²'],['parabola','The parabola','y²=4ax'],['ellipse','The ellipse','x²/a²'],['hyperbola','The hyperbola','x²−y²'],['tangents','Tangents & chords','T=0'],['polepolar','Pole & polar','T=S₁'],['generalconic','General conic','B²−4AC'],['eccentricity','Focus & eccentricity','e'],['locus','Loci & parameters','P(t)'],['hybrid','Hybrids','⊕']];
  ...
  function renderList(view, list, title, glyph){
    let html='<div class="chap-hero card"><div class="ch-glyph-lg">S=0</div>'
      + '<div class="ch-rom">THE COMPLETE SET</div><h2>'+esc(title)+'</h2>'
      + '<div class="ch-desc">Every conic-sections problem in the book, in chapter order. Filter by difficulty, or search from the bar above.</div>'
