/* ============================================================
   y=mx+c — Slopes, Distances & the Pencil of Lines  ·  application logic
   ============================================================ */
(function () {
  'use strict';
  ...
  const THEME_META = {
    ...
    anglefamily:     { glyph:'L₁+λL₂', blurb:'The angle between two lines, parallel and perpendicular conditions, concurrency of three lines, and the family L₁+λL₂ through a fixed intersection.' },
  ...
  // ---- store ----
  function loadStore() {
    try { return JSON.parse(localStorage.getItem('lines_masterbook') || '{}'); }
    catch (e) { return {}; }
  }
  function saveStore() {
    try { localStorage.setItem('lines_masterbook', JSON.stringify(store)); } catch (e) {}
  }
  ...
  function renderList(view, list, title, glyph){
    let html='<div class="chap-hero card"><div class="ch-glyph-lg">y=mx+c</div>'
      + '<div class="ch-rom">THE COMPLETE SET</div><h2>'+esc(title)+'</h2>'
      + '<div class="ch-desc">Every straight-line problem in the book, in chapter order. Filter by difficulty, or search from the bar above.</div>'
