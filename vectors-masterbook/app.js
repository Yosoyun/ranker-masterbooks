/* ============================================================
   v⃗ — Vectors & the Geometry of Space  ·  application logic
   ============================================================ */
(function () {
  'use strict';
  ...
  const THEME_META = {
    ...
    tripleproduct: { glyph:'[abc]',  blurb:'The box product as a signed volume, the clean test for coplanarity, and the BAC–CAB expansion that unravels a nested cross product.' },
    lineardep:     { glyph:'λa+μb',  blurb:'When vectors are independent, when they collapse into a plane or a line, expressing one in a basis, and the reciprocal system that inverts the dot product.' },
    position:      { glyph:'r⃗',     blurb:'Points as vectors from an origin — the section formula, centroids and the collinearity test, and classic geometry proved without a single coordinate.' },
    lines3d:       { glyph:'ℓ',      blurb:'The vector and Cartesian forms of a line, intersection and skewness, the foot of the perpendicular, and the image of a point in a line.' },
    planes:        { glyph:'Π',      blurb:'The normal form of a plane, the angle between two planes, their line of intersection, and the pencil of planes through a common line.' },
  ...
  // ---- store ----
  function loadStore() {
    try { return JSON.parse(localStorage.getItem('vectors_masterbook') || '{}'); }
    catch (e) { return {}; }
  }
  function saveStore() {
    try { localStorage.setItem('vectors_masterbook', JSON.stringify(store)); } catch (e) {}
  }
  ...
  function renderHome(view){
    ...
    const fam=[['dotproduct','Dot product & projections','a·b'],['crossproduct','Cross product & areas','a×b'],['tripleproduct','Triple products','[abc]'],['lineardep','Bases & dependence','λa+μb'],['position','Position & section','r⃗'],['lines3d','Lines in space','ℓ'],['planes','Planes & pencils','Π'],['distances','Distances & skew','⊥'],['loci','Loci & spheres','|r−a|'],['hybrid','Hybrids','⊕']];
  ...
  function renderList(view, list, title, glyph){
    let html='<div class="chap-hero card"><div class="ch-glyph-lg">v&#8407;</div>'
      + '<div class="ch-rom">THE COMPLETE SET</div><h2>'+esc(title)+'</h2>'
      + '<div class="ch-desc">Every vectors &amp; 3D problem in the book, in chapter order. Filter by difficulty, or search from the bar above.</div>'
