/* ============================================================
   Σ — Patterns, Sums & Limits  ·  application logic
   ============================================================ */
(function () {
  'use strict';
  ...
  // ---- store ----
  function loadStore() {
    try { return JSON.parse(localStorage.getItem('sequences_masterbook') || '{}'); }
    catch (e) { return {}; }
  }
  function saveStore() {
    try { localStorage.setItem('sequences_masterbook', JSON.stringify(store)); } catch (e) {}
  }
  ...
  function renderList(view, list, title, glyph){
    let html='<div class="chap-hero card"><div class="ch-glyph-lg">Σ</div>'
      + '<div class="ch-rom">THE COMPLETE SET</div><h2>'+esc(title)+'</h2>'
      + '<div class="ch-desc">Every sequences &amp; series problem in the book, in chapter order. Filter by difficulty, or search from the bar above.</div>'
