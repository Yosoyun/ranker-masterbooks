/* ============================================================
   f′=0 — Tangents, Extrema & the Shape of a Curve  ·  application logic
   ============================================================ */
(function () {
  'use strict';
  ...
  const THEME_META = {
    ...
    monotonicity:  { glyph:'f′>0',  blurb:'Where a function increases or decreases from the sign of f′, proving inequalities by monotonicity, and finding the parameter range that keeps a function monotone.' },
    maxmin:        { glyph:'f′=0',  blurb:'Local and global maxima and minima — the first and second derivative tests, endpoints and non-differentiable points, and extrema under a constraint.' },
    rollelmvt:     { glyph:'f′(c)', blurb:'Rolle\'s theorem and the Mean Value Theorem — existence of a root of f′, bounding a function from its derivative, and the consequences for uniqueness of roots.' },
    approxerror:   { glyph:'df',    blurb:'Differentials and linear approximation — estimating a value, the percentage error propagated through a formula, and Newton-style refinement.' },
    concavity:     { glyph:'f″',    blurb:'Concavity and points of inflection from the sign of f″, the shape they impose, and the inequalities (Jensen-flavoured) that convexity proves.' },
  ...
  // ---- store ----
  function loadStore() {
    try { return JSON.parse(localStorage.getItem('aod_masterbook') || '{}'); }
    catch (e) { return {}; }
  }
  function saveStore() {
    try { localStorage.setItem('aod_masterbook', JSON.stringify(store)); } catch (e) {}
  }
  ...
  function renderHome(view){
    ...
    const fam=[['tangentnormal','Tangents & normals','m=f′'],['ratemeasure','Rates of change','dy/dt'],['monotonicity','Monotonicity','f′>0'],['maxmin','Maxima & minima','f′=0'],['rollelmvt','Rolle & MVT','f′(c)'],['approxerror','Approximations','df'],['concavity','Concavity & inflection','f″'],['curvesketch','Curve sketching','y=f(x)'],['optimization','Optimisation','min S'],['hybrid','Hybrids','⊕']];
  ...
  function renderList(view, list, title, glyph){
    let html='<div class="chap-hero card"><div class="ch-glyph-lg">f&prime;=0</div>'
      + '<div class="ch-rom">THE COMPLETE SET</div><h2>'+esc(title)+'</h2>'
      + '<div class="ch-desc">Every application-of-derivatives problem in the book, in chapter order. Filter by difficulty, or search from the bar above.</div>'
