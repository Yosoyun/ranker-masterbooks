/* ============================================================
   ARC — The Inverse Circular Functions  ·  application logic
   ============================================================ */
(function () {
  'use strict';
  ...
  const THEME_META = {
    ...
    telescope:   { glyph:'Σ',     blurb:'Write each term as a difference of two angles, and watch an infinite sum collapse to one.' },
    identity:    { glyph:'≡',     blurb:'arcsin x + arccos x is a constant. Most nested inverse-trig expressions are secretly a straight line.' },
    equations:   { glyph:'=',     blurb:'Every inverse-trig equation is a domain problem in disguise. Count only the roots the graph allows.' },
  ...
  // ---- store ----
  function loadStore() {
    try { return JSON.parse(localStorage.getItem('itf_masterbook') || '{}'); }
    catch (e) { return {}; }
  }
  function saveStore() {
    try { localStorage.setItem('itf_masterbook', JSON.stringify(store)); } catch (e) {}
  }
  ...
  function renderHome(view){
    ...
    html+='<div class="cover">'
      ...
      + '<div class="cover-cta">'
        ...
        + '<a class="btn ghost" href="ARC-Problems.pdf" download>&#8595; Problems PDF</a>'
        + '<a class="btn ghost" href="ARC-Solutions.pdf" download>&#8595; Solutions PDF</a>'
      + '</div>'
  ...
  function renderList(view, list, title, glyph){
    let html='<div class="chap-hero card"><div class="ch-glyph-lg">sin⁻¹</div>'
      + '<div class="ch-rom">THE COMPLETE SET</div><h2>'+esc(title)+'</h2>'
      + '<div class="ch-desc">Every problem in the book, in chapter order. Filter by difficulty, or search from the bar above.</div>'
  ...
  function buildCard(p){
    ...
    st.innerHTML='<span class="stmt-label">'+esc(p.task||'Evaluate')+'</span><div class="stmt-math"></div>';
    ...
    ansBox.innerHTML='<span class="ans-label">'+esc(p.answerLabel||'Answer')+'</span><span class="ans-val"></span>';
