...
window.PROBLEMS = [
  ...
  },
  {
    ...
    "statement": "For real constants $a,b,c>0$ the exponential Frullani integral $\\displaystyle\\int_{0}^{\\infty}\\frac{e^{-ax}-e^{-bx}}{x}\\,dx=\\ln\\frac{b}{a}$ is a chapter classic. Strangle it with a fixed-frequency cosine. With $a=1,\\ b=7,\\ c=1$ evaluate \\[ I=\\int_{0}^{\\infty}\\frac{e^{-ax}-e^{-bx}}{x}\\,\\cos(cx)\\,dx. \\] The weight $\\cos(cx)$ oscillates with a frequency $c$ that is shared by both exponential terms and does \\text{not} rescale when $a$ becomes $b$, so the integrand is genuinely \\text{not} of Frullani form $\\dfrac{f(ax)-f(bx)}{x}$. Recover the value by combining the Frullani endpoint structure with a Laplace/parameter argument that respects the oscillation, then report $I$ for the given data.",
    "answer": "\\[\\boxed{\\,I=\\ln 5\\,}\\]",
  ...
  },
  {
    ...
    "statement": "For a real parameter $a\\neq0$ define the Poisson-kernel log integral \\[ \\Phi(a)=\\int_{0}^{\\pi}\\ln\\!\\bigl(1-2a\\cos x+a^{2}\\bigr)\\,dx, \\] a famous companion to Euler's $\\int_0^{\\pi/2}\\ln(\\sin x)\\,dx$. Three sibling integrals are placed in front of you; evaluate each in closed form. (a) $\\displaystyle A=\\int_{0}^{\\pi}\\ln\\bigl(5-4\\cos x\\bigr)\\,dx.$ (b) $\\displaystyle B=\\int_{0}^{\\pi}\\ln\\!\\Bigl(\\tfrac{5}{4}-\\cos x\\Bigr)\\,dx.$ (c) $\\displaystyle C=\\int_{0}^{\\pi}\\ln\\bigl(2-2\\cos x\\bigr)\\,dx.$ Each quadratic-in-$\\cos x$ is of the form $1-2a\\cos x+a^{2}$ for a specific $a$; identify that $a$, and BEFORE quoting any formula decide where $a$ sits relative to the unit circle $|a|=1$, because the value of $\\Phi(a)$ is governed entirely by that verdict. Report the ordered triple $(A,B,C)$.",
