/* ============================================================
   ∫ — The Definite Integral  ·  application logic
   ============================================================ */
(function () {
  'use strict';
  ...
  const THEME_META = {
    ...
    riemann:       { glyph:'Σ→∫',  blurb:'A sum of a thousand terms is an integral in disguise — read its limits straight off the index.' },
    reduction:     { glyph:'Iₙ',   blurb:'Integration by parts turns the nth power into the (n−2)th, and Wallis closes the loop.' },
  ...
  // ---- store ----
  function loadStore() {
    try { return JSON.parse(localStorage.getItem('definite_masterbook') || '{}'); }
    catch (e) { return {}; }
  }
  function saveStore() {
    try { localStorage.setItem('definite_masterbook', JSON.stringify(store)); } catch (e) {}
  }
  ...
  function renderHome(view){
    ...
    const fam=[['properties','Symmetry & parity','∫ₐᵇ'],['kingrule','The king rule','a↔b'],['riemann','Limit of sums','Σ→∫'],['reduction','Reduction & Wallis','Iₙ'],['leibniz','Under the integral','F′'],['estimation','Estimation & MVT','≤'],['periodicfloor','Floor & period','⌊x⌋'],['famous','Famous integrals','lnsin'],['improper','Improper integrals','∞'],['hybrid','Hybrids','⊕']];
  ...
  function renderList(view, list, title, glyph){
    let html='<div class="chap-hero card"><div class="ch-glyph-lg">&#8747;</div>'
      ...
      + '<div class="ch-desc">Every definite-integral problem in the book, in chapter order. Filter by difficulty, or search from the bar above.</div>'
