/* ============================================================
   ∫dx — The Antiderivative & Its Constant  ·  application logic
   ============================================================ */
(function () {
  'use strict';
  ...
  const THEME_META = {
    ...
    byparts:         { glyph:'∫u dv',     blurb:'Integration by parts and the ILATE choice, the loop ∫=…−∫ that solves itself, and the eˣ and reduction patterns it unlocks.' },
    partialfractions:{ glyph:'A/(x−a)',   blurb:'Rational functions broken into partial fractions — repeated and irreducible-quadratic factors, the cover-up method, and the log/arctan pieces that result.' },
    trigintegrals:   { glyph:'∫sinⁿ',     blurb:'Powers and products of sines and cosines, the odd/even strategy, and the Weierstrass substitution t=tan(x/2) that rationalises everything.' },
    irrational:      { glyph:'∫dx/√Q',    blurb:'Integrands with a square root of a quadratic — completing the square, the right trig/hyperbolic substitution, and Euler\'s substitutions.' },
    specialdenoms:   { glyph:'1/(a+bcos)', blurb:'The special denominators — a+b cos x, a+b sin x, a sin²x+b cos²x, and (px+q)√(ax²+bx+c) — each with its own decisive trick.' },
  ...
  // ---- store ----
  function loadStore() {
    try { return JSON.parse(localStorage.getItem('indefinite_masterbook') || '{}'); }
    catch (e) { return {}; }
  }
  function saveStore() {
    try { localStorage.setItem('indefinite_masterbook', JSON.stringify(store)); } catch (e) {}
  }
  ...
  function renderHome(view){
    ...
    const fam=[['standard','Standard forms','∫xⁿ'],['substitution','Substitution','∫f(g)g′'],['byparts','By parts','∫u dv'],['partialfractions','Partial fractions','A/(x−a)'],['trigintegrals','Trigonometric','∫sinⁿ'],['irrational','Irrational','∫dx/√Q'],['specialdenoms','Special denominators','1/(a+b cos)'],['reduction','Reduction formulae','Iₙ'],['exactderiv','Exact derivatives','eˣ(f+f′)'],['hybrid','Hybrids','⊕']];
  ...
  function renderList(view, list, title, glyph){
    let html='<div class="chap-hero card"><div class="ch-glyph-lg">&int;dx</div>'
      + '<div class="ch-rom">THE COMPLETE SET</div><h2>'+esc(title)+'</h2>'
      + '<div class="ch-desc">Every indefinite-integration problem in the book, in chapter order. Filter by difficulty, or search from the bar above.</div>'
