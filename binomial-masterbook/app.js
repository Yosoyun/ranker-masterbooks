/* ============================================================
   (1+x)ⁿ — The Expansion & Its Coefficients  ·  application logic
   ============================================================ */
(function () {
  'use strict';
  ...
  const THEME_META = {
    ...
    coefficients:   { glyph:'[xᵏ]',  blurb:'Extracting the coefficient of a chosen power from a product, a quotient, or a power of a polynomial — the coefficient as a count.' },
    sumsdiff:       { glyph:'ΣkⁿCₖ', blurb:'Differentiate or integrate (1+x)ⁿ and read off a sum no one would dare add by hand — calculus as a summation engine.' },
    rootsofunity:   { glyph:'ω',     blurb:'Sift out every third (or k-th) coefficient by averaging the expansion over the roots of unity — the filter that splits a sum.' },
  ...
  // ---- store ----
  function loadStore() {
    try { return JSON.parse(localStorage.getItem('binomial_masterbook') || '{}'); }
    catch (e) { return {}; }
  }
  function saveStore() {
    try { localStorage.setItem('binomial_masterbook', JSON.stringify(store)); } catch (e) {}
  }
  ...
  function renderList(view, list, title, glyph){
    let html='<div class="chap-hero card"><div class="ch-glyph-lg">[xᵏ]</div>'
      + '<div class="ch-rom">THE COMPLETE SET</div><h2>'+esc(title)+'</h2>'
      + '<div class="ch-desc">Every binomial-theorem problem in the book, in chapter order. Filter by difficulty, or search from the bar above.</div>'
