/* problems.js — DATA. 100 original JEE Advanced complex-numbers problems for ℂ · The Argand Plane. Adversarially verified in Python (sympy/numpy/mpmath). statement/answer raw LaTeX (app auto-detects prose+math); steps use $...$ and $$...$$. */
window.PROBLEMS = [
  {
    "theme": "algebra",
  ...
  },
  {
    ...
    "statement": "Work on the extended plane and consider the single map \\[ w=\\frac1z . \\] Call a line or circle $\\Gamma$ self-mapping if its image under $w=\\tfrac1z$ is $\\Gamma$ again as a set (every point of $\\Gamma$ lands on $\\Gamma$, and every point of $\\Gamma$ is the image of a point of $\\Gamma$). Determine, with proof, all self-mapping lines and all self-mapping circles. For the circles, characterise the admissible centre $c$ and radius $r>0$ of $|z-c|=r$ exactly. (A tempting half-answer: ``the self-mapping circles are precisely those orthogonal to the unit circle, i.e. $|c|^2-r^2=1$.'' Decide whether this is the complete truth, and correct it if it is not.)",
  ...
  },
  {
    ...
    "statement": "Work in the extended complex plane and let $w=\\dfrac1z$. Consider the closed planar region cut out by two curves at once, \\[ \\mathcal{R}=\\{\\,z:\\operatorname{Re}(z)\\ge 1\\,\\}\\;\\cap\\;\\{\\,z:|z-1|\\le 1\\,\\}, \\] a lune whose left edge is the vertical chord on the line $\\operatorname{Re}(z)=1$ joining $1-i$ to $1+i$, and whose right edge is the arc of the circle $|z-1|=1$ through the point $z=2$. (a) Find the image of each bounding curve separately under $w=1/z$, being explicit about which becomes a line and which becomes a circle, and explain geometrically why exactly one of them straightens out. (b) Combine the two boundary images with the correct interior-side bookkeeping to identify the full image region $w(\\mathcal{R})$, naming it as a recognizable elementary region. (c) Using the conformality of $w=1/z$ away from the pole, state the interior angle of $w(\\mathcal{R})$ at each of its two curvilinear corners, and report the area of $w(\\mathcal{R})$. Give as the final answer the description of $w(\\mathcal{R})$ together with its area.",
  ...
  },
  {
    ...
    "statement": "Fix an interior point $a$ of the open unit disk, $0<|a|<1$. We seek a M\\\"obius transformation $w$ of the extended plane that is simultaneously all of the following: (i) an involution --- $w(w(z))=z$ for every $z$, but $w$ is not the identity; (ii) it carries the unit circle $|z|=1$ to itself as a set (and hence maps the open disk to a fixed region); (iii) it swaps the centre and the star: $w(0)=a$ and $w(a)=0$. Find $w$ explicitly in terms of $a$ and prove it is the only M\\\"obius map meeting (i)--(iii). Then identify the two fixed points of $w$ and state their precise location relative to the unit circle. (A natural first move: ``an involution that keeps the circle setwise and swaps two real-axis points must keep the two points where the diameter through $0$ and $a$ pierces the circle fixed.'' Decide whether that pins down $w$, and if not, say what it actually produces.)",
  ...
  },
  {
    ...
    "statement": "Work in the extended complex plane under the inversion \\[ w=\\frac1z . \\] Let $\\mathcal{C}$ be the circle \\[ \\mathcal{C}:\\ |z-3|=1, \\] which lies wholly in the right half-plane and does not pass through the pole $z=0$. (a) Find the image curve $w(\\mathcal{C})$ exactly: prove it is a circle and give its centre and radius. (b) The point $z=3$ is the centre of $\\mathcal{C}$. Decide whether its image $w(3)$ is the centre of the image circle $w(\\mathcal{C})$; if not, compute the exact distance between $w(3)$ and the true centre of $w(\\mathcal{C})$.",
  ...
  },
  {
    ...
    "statement": "Let \\[ P(z)=z^{6}-3z^{4}+28z^{3}-20z^{2}-200z+500 \\] regarded as a polynomial over $\\mathbb{C}$ with real coefficients (note that the $z^{5}$ term is absent, so the six roots sum to $0$). (a) Prove that every critical point of $P$ — that is, every root of $P'(z)$ — lies in the convex hull of the six roots of $P$ in the complex plane (the Gauss--Lucas theorem for this specific $P$), and verify that $P$ has no root on the imaginary axis. (b) Using the argument principle (equivalently, the Routh--Hurwitz criterion), determine exactly how many roots of $P$ lie in the open right half-plane $\\{\\,z:\\operatorname{Re}z>0\\,\\}$. Report that integer as the final answer.",
    "answer": "\\[\\boxed{4}\\]",
  ...
  },
  {
    ...
    "statement": "Let \\[ P(z)=z^{6}+a\\,z^{5}+b\\,z^{4}+c\\,z^{3}+b\\,z^{2}+a\\,z+1 \\] be a self-reciprocal (palindromic) polynomial with real coefficients $a,b,c$; thus $z^{6}P(1/z)=P(z)$, so the non-zero roots of $P$ come in reciprocal pairs $\\{r,\\,1/r\\}$. You are told three facts: (i) $w=\\tfrac{3}{5}+\\tfrac{4}{5}i$ is a root of $P$; (ii) $\\zeta=e^{2\\pi i/3}=-\\tfrac12+\\tfrac{\\sqrt3}{2}i$ is a root of $P$; (iii) $P(1)+P(-1)=10$. Determine $a,b,c$, prove that all six roots of $P$ lie on the unit circle $|z|=1$, and list them. Be warned: the on-circle conclusion is not automatic from palindromicity plus the numerical datum — it must be earned.",
  ...
  },
  {
    ...
    "statement": "Start from the seven seventh roots of unity, the solutions of $z^{7}=1$. Stripping the root $z=1$ leaves the palindromic sextic \\[ z^{6}+z^{5}+z^{4}+z^{3}+z^{2}+z+1=0, \\] whose six roots are $e^{\\pm 2\\pi i/7},\\,e^{\\pm 4\\pi i/7},\\,e^{\\pm 6\\pi i/7}$. (a) By dividing through by $z^{3}$ and substituting $y=z+\\tfrac1z$, fold the sextic into a real cubic in $y$, and hence prove that $x=\\cos\\tfrac{2\\pi}{7}$ satisfies the integer cubic \\[ 8x^{3}+4x^{2}-4x-1=0, \\] establishing that this is the minimal polynomial of $\\cos\\tfrac{2\\pi}{7}$ over $\\mathbb{Q}$ (you may take its irreducibility as given). (b) The three roots of that cubic are exactly $\\cos\\tfrac{2\\pi}{7},\\,\\cos\\tfrac{4\\pi}{7},\\,\\cos\\tfrac{6\\pi}{7}$. Using only the cubic and Vieta's relations — never a decimal value of any cosine — evaluate the sum of secants \\[ S=\\sec\\tfrac{2\\pi}{7}+\\sec\\tfrac{4\\pi}{7}+\\sec\\tfrac{6\\pi}{7}. \\] Report $S$ as the final answer.",
    "answer": "\\[\\boxed{-4}\\]",
  ...
  },
  {
    ...
    "statement": "Let $n\\ge 2$ and let $\\omega_{1},\\dots,\\omega_{n-1}$ be the $n-1$ non-trivial $n$-th roots of unity, i.e. the roots of \\[ Q(z)=\\frac{z^{n}-1}{z-1}=1+z+z^{2}+\\cdots+z^{n-1}. \\] Consider the resolvent sum taken at the puncture $z=1$, \\[ S(n)=\\sum_{k=1}^{n-1}\\frac{1}{\\lvert 1-\\omega_{k}\\rvert^{2}}. \\] (a) Prove the closed form $\\displaystyle S(n)=\\frac{n^{2}-1}{12}$ by differentiating the relation $Q'(z)/Q(z)=\\sum_{k}1/(z-\\omega_{k})$ at $z=1$ — not by re-deriving any chord-length product. (b) Hence evaluate $S(13)$, and report that single number as the final answer. The modulus bars are the whole point: $\\lvert 1-\\omega_{k}\\rvert^{2}=(1-\\omega_{k})(1-\\overline{\\omega_{k}})$ mixes a root with the conjugate root, so the quantity you must control is genuinely Hermitian, not the holomorphic square $1/(1-\\omega_{k})^{2}$.",
    "answer": "\\[\\boxed{14}\\]",
  ...
  },
  {
    ...
    "statement": "The Cayley map $w=\\dfrac{z-i}{z+i}$ is famous for one thing: it carries the real axis bijectively onto the unit circle $\\lvert w\\rvert=1$. We now feed it a different curve. Let $z=x+iy$ be constrained to run over the right-hand branch of the rectangular hyperbola \\[ \\operatorname{Re}(z^{2})=1,\\qquad\\text{i.e.}\\qquad x^{2}-y^{2}=1,\\ \\ x\\ge 1, \\] and let $w$ be its image under the Cayley map. (a) Show that the image is the quartic curve \\[ \\bigl(\\lvert w\\rvert^{2}\\bigr)^{2}-2\\,(\\operatorname{Re}w)\\,\\lvert w\\rvert^{2}+2\\bigl((\\operatorname{Re}w)^{2}-(\\operatorname{Im}w)^{2}\\bigr)-2\\operatorname{Re}w+1=0, \\] and in particular that it is $not$ an arc of the unit circle. (b) Find the point (or points) of this image curve nearest the origin of the $w$-plane, and report the minimal distance $\\displaystyle \\min\\lvert w\\rvert$ as the final answer. Throughout, $\\lvert w\\rvert^{2}=\\dfrac{\\lvert z-i\\rvert^{2}}{\\lvert z+i\\rvert^{2}}$ is the natural object to track; the constraint $x^{2}=1+y^{2}$ should be used to collapse it to a one-variable problem before any minimization.",
  ...
  },
  {
    ...
    "statement": "Place unit charges at the seven vertices of a regular heptagon, taken to be the seventh roots of unity $z_0,\\dots,z_6$ with $z_k=e^{2\\pi i k/7}$ on the unit circle. For each of the $\\binom{7}{2}=21$ chords joining a pair of vertices, record two numbers: its inverse-square length $w_{jk}=\\dfrac{1}{\\lvert z_j-z_k\\rvert^{2}}$ (the Coulomb energy of that pair), and the position $M_{jk}=\\dfrac{z_j+z_k}{2}$ of its midpoint. Two preliminary facts must be established first, and both are needed for the finale: \\\\ (i) the total energy $\\displaystyle E_7=\\sum_{j<k} w_{jk}=\\sum_{j<k}\\frac{1}{\\lvert z_j-z_k\\rvert^{2}}$ admits the closed form coming from the cosecant identity $\\sum_{m=1}^{6}\\csc^{2}\\dfrac{m\\pi}{7}=\\dfrac{7^{2}-1}{3}$; \\\\ (ii) the centroid of the $21$ chord-midpoints, $\\displaystyle G=\\frac{1}{21}\\sum_{j<k}M_{jk}$, coincides with the centre of the polygon, i.e. $G=0$. Using (ii) to certify that the midpoint cloud is centred at the origin, define the energy-weighted mean-square radius of that cloud about the centre, \\[ Q \\;=\\; \\frac{\\displaystyle\\sum_{j<k} w_{jk}\\,\\lvert M_{jk}\\rvert^{2}}{\\displaystyle\\sum_{j<k} w_{jk}} \\;=\\; \\frac{1}{E_7}\\sum_{j<k}\\frac{\\lvert M_{jk}\\rvert^{2}}{\\lvert z_j-z_k\\rvert^{2}}. \\] Compute the exact value of $Q$. (You will need both the cosecant census of (i) for the denominator and a companion cotangent census for the numerator; the centroid fact (ii) is what guarantees this scalar is the genuine spread about the centre, with no parallel-axis correction.)",
  ...
  },
  {
    ...
    "statement": "Fix an integer $n\\ge 1$ and consider the $n$ acute angles \\[ \\theta_k=\\frac{(2k+1)\\pi}{4n},\\qquad k=0,1,\\dots,n-1, \\] so that $\\theta_0<\\theta_1<\\dots<\\theta_{n-1}$ all lie in $\\left(0,\\tfrac{\\pi}{2}\\right)$. (a) By examining the real part of $(1+ix)^{2n}$, build the monic-up-to-scale polynomial of degree $n$ whose roots are exactly the numbers $\\tan^2\\theta_k$, and use it to prove the closed form \\[ S_n:=\\sum_{k=0}^{n-1}\\cot^2\\!\\theta_k=\\sum_{k=0}^{n-1}\\cot^2\\!\\frac{(2k+1)\\pi}{4n}=n(2n-1). \\] (b) Using only the elementary squeeze $\\cot^2\\theta<\\dfrac{1}{\\theta^2}<\\csc^2\\theta$ valid for $0<\\theta<\\tfrac{\\pi}{2}$, together with the value of $S_n$, deduce that the partial odd-Basel sums obey, for every $n\\ge1$, the strict two-sided bound \\[ \\frac{4}{3}\\sum_{k=0}^{n-1}\\frac{1}{(2k+1)^2}\\;<\\;\\frac{\\pi^2}{6}\\;<\\;\\frac{8n}{3(2n-1)}\\sum_{k=0}^{n-1}\\frac{1}{(2k+1)^2}, \\] so that letting $n\\to\\infty$ recovers $\\displaystyle\\sum_{m=1}^{\\infty}\\frac{1}{m^2}=\\frac{\\pi^2}{6}$. Report the closed form of part (a) as the final answer.",
