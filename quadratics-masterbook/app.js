/* ============================================================
   x² — Roots, Discriminant & Vieta  ·  application logic
   ============================================================ */
(function () {
  'use strict';
  ...
  const THEME_META = {
    ...
    location:      { glyph:'f(k)',   blurb:'Pin the roots against the number line with a single sign of a·f(k), the vertex and the discriminant.' },
    commonroots:   { glyph:'∩',      blurb:'When two quadratics share a root, elimination hands you both the root and the hidden parameter.' },
    rangefraction: { glyph:'y=',     blurb:'Turn a rational expression into a quadratic in x and demand a real solution — the discriminant draws the range.' },
    maxmin:        { glyph:'⌣',      blurb:'Every parabola has one turning point. Complete the square — but watch the interval’s edge.' },
    disguise:      { glyph:'t=x²',   blurb:'Biquadratics, reciprocals, exponentials — the right substitution unmasks a quadratic underneath.' },
  ...
  // ---- store ----
  function loadStore() {
    try { return JSON.parse(localStorage.getItem('quadratics_masterbook') || '{}'); }
    catch (e) { return {}; }
  }
  function saveStore() {
    try { localStorage.setItem('quadratics_masterbook', JSON.stringify(store)); } catch (e) {}
  }
  ...
  function renderHome(view){
    ...
    html+='<div class="cover">'
      ...
      + '<div class="cover-cta">'
        ...
        + '<a class="btn ghost" href="https://github.com/Yosoyun/ranker-masterbooks/releases/download/pdfs/QUAD-Problems.pdf" download>&#8595; Problems PDF</a>'
        + '<a class="btn ghost" href="https://github.com/Yosoyun/ranker-masterbooks/releases/download/pdfs/QUAD-Solutions.pdf" download>&#8595; Solutions PDF</a>'
      + '</div>'
      + '<p class="cover-manifesto">Vieta and the symmetric functions of the roots, the discriminant, the location of roots, common roots, the range via the discriminant, maxima and minima, equations reducible to a quadratic, parameter problems and quadratic inequalities — each one a doorway to a problem that punishes the careless. Every problem here is solved more than one way, because a ranker doesn’t memorise the formula; they read the parabola.</p>'
      + '</div>';
    ...
    const fam=[['vieta','Vieta','α+β'],['discriminant','Discriminant','Δ'],['location','Location of roots','f(k)'],['commonroots','Common roots','∩'],['rangefraction','Range','y='],['maxmin','Max & min','⌣'],['disguise','Reducible','t=x²'],['parameter','Parameters','k'],['inequalities','Inequalities','>0'],['hybrid','Hybrids','⊕']];
  ...
  function renderList(view, list, title, glyph){
    let html='<div class="chap-hero card"><div class="ch-glyph-lg">x²</div>'
